  ...
  Future<void> saveHabit(
    ...
    try {
      ...
      final maxOrderResult = await db.rawQuery(
        'SELECT MAX(display_order) as max_order FROM habits',
      );
      final maxOrder =
          maxOrderResult.isNotEmpty
              ? (maxOrderResult.first['max_order'] as int? ?? 0)
              : 0;
...
class _HabitsScreenState extends State<HabitsScreen> {
  ...
  DateTime _currentLogMonth = DateTime(
    DateTime.now().year,
    DateTime.now().month,
  );
        ...
        title:
            _showSearch
                ? TextField(
                  ...
                  onChanged:
                      (v) => context.read<HabitsProvider>().setSearchQuery(v),
                )
              ...
              child: Column(
                ...
                children: [
                  ...
                  FilledButton.tonalIcon(
                    onPressed: () => context.read<HabitsProvider>().load(),
                ...
                if (provider.isSelectionMode)
                  Container(
                    ...
                    child: Row(
                      children: [
                        Text(
                          '${provider.selectedHabits.length} selected',
                    ...
                    final discard = await showDialog<bool>(
                      ...
                      builder:
                          (ctx) => AlertDialog(
                            ...
                            content: const Text(
                              'You have unsaved changes. Are you sure you want to discard this habit?',
                            ),
                            actions: [
                              TextButton(
    ...
    showDialog(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Text(
              'Are you sure you want to delete "${habit.name}" and all of its progress logs?',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(ctx),
      ...
      child: Row(
        ...
        children: List.generate(7, (index) {
          ...
          return Expanded(
            child: Semantics(
              label:
                  '${dayLabels[index]} ${date.day}${isFuture ? ", cannot log future dates" : ""}',
              button: !isFuture,
              child: GestureDetector(
                onTap:
                    isFuture ? null : () => provider.toggleLog(habit.id!, date),
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Text(
                ...
                children: [
                  Expanded(
                    child: _StatBlock(
                      ...
                      subtitle:
                          '${(completedCount / totalDays * 100).toInt()}%',
                      color: habitColor,
                    ),
                  ),
                  Expanded(
...
class _HabitIconPickerState extends State<_HabitIconPicker> {
  ...
  List<String> get _filteredIcons =>
      _iconCategories[_selectedCategory] ?? _allIconKeys;
